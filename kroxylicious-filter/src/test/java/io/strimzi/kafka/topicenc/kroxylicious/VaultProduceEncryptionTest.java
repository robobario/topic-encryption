...
class VaultProduceEncryptionTest {
    ...
    @Test
    public void testProduceEncryption(Admin admin) {
        try (Producer<String, String> producer = tester.producer(Serdes.String(), Serdes.String(), Map.of());
             Consumer<String, byte[]> consumer = tester.consumer(Serdes.String(), Serdes.ByteArray(), Map.of(ConsumerConfig.GROUP_ID_CONFIG, "another-group-id", ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, "earliest"))
        ) {
            ...
            assertSingletonRecordEquals(consumer, TOPIC_NAME_A, (s) -> decrypt(s, TOPIC_NAME_A), UNENCRYPTED_VALUE);
            assertSingletonRecordEquals(consumer, TOPIC_NAME_B, (s) -> decrypt(s, TOPIC_NAME_B), UNENCRYPTED_VALUE);
        } catch (Exception e) {
